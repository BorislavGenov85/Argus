...
const portCount = {ports: 0, dirs: 0, dns: 0};
...
function updateStage(stage, status) {
    ...
    if (!el) return;
    el.className = 'stage';
    if (status === 'started' || status === 'result') el.classList.add('active');
    if (status === 'done' || status === 'skipped') el.classList.add('done');
    if (status === 'error') el.classList.add('error');
}

function addPortRow(msg) {
    const tbody = document.getElementById('ports-body');
    if (portCount.ports === 0) tbody.innerHTML = '';

    const httpBadge = msg.is_http ? '<span class="http-badge">HTTP</span>' : '';
    const tr = document.createElement('tr');
    tr.innerHTML = `
            <td><span class="port-num">${msg.port}</span>/${msg.protocol} ${httpBadge}</td>
            <td>${msg.protocol}</td>
            <td class="service-name">${msg.service || '—'}</td>
            <td>${[msg.product, msg.version].filter(Boolean).join(' ') || '—'}</td>
        `;
...
function addDirRow(msg) {
    ...
    if (portCount.dirs === 0) tbody.innerHTML = '';

    const statusClass = `status-${msg.status_code}`;
    const tr = document.createElement('tr');
    tr.innerHTML = `
            <td><a href="${msg.url}" target="_blank" style="color:var(--accent2)">${msg.url}</a></td>
            <td class="${statusClass}">${msg.status_code}</td>
            <td>${msg.size} B</td>
        `;
    tbody.appendChild(tr);
    portCount.dirs++;
...
function addDnsRow(msg) {
    ...
    if (portCount.dns === 0) tbody.innerHTML = '';

    const tr = document.createElement('tr');
    tr.innerHTML = `
            <td style="color:var(--accent2)">${msg.subdomain}</td>
            <td style="color:#c084fc">${msg.record_type}</td>
            <td>${msg.value}</td>
        `;
    tbody.appendChild(tr);
    portCount.dns++;
...
async function clearDB() {
    ...
    document.getElementById('sessionList').innerHTML = '<div class="empty-state">No scans</div>';
}
